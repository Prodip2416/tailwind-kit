...
import { formsComponentsData } from '../components/forms/formsComponentsData';
import FormsElementRenderer from '../components/forms/FormsElementRenderer';
import { generateFormsCodeExample } from '../components/forms/FormsCodeGenerator';
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(
                      generateFormsCodeExample(component, codeLanguage)
                    );
                    setCopied(true);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="space-y-8">
          {formType
            ? (() => {
                const component = formsComponentsData.find(
                  (comp) => comp.id === formType
                );
                if (!component) return <div>Form component not found</div>;
                return <FormCard component={component} />;
              })()
            : formsComponentsData.map((component) => (
                <FormCard key={component.id} component={component} />
              ))}
        </div>
      </div>
    </div>
