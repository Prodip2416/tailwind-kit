...
import { layoutComponentsData } from '../components/layout/layoutComponentsData';
import LayoutElementRenderer from '../components/layout/LayoutElementRenderer';
import { generateLayoutCodeExample } from '../components/layout/LayoutCodeGenerator';

const LayoutCard = ({ component }) => {
      ...
      <div className="flex gap-4">
        ...
        <div className="w-2/3 min-w-0 flex flex-col">
          <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md border border-gray-200 dark:border-slate-700 p-4 flex flex-col h-full">
            ...
            <div className="flex-1 min-h-0 overflow-auto border border-gray-200 dark:border-slate-600 rounded-lg p-3">
              <LayoutElementRenderer layoutType={component.id} />
            </div>
          </div>
        </div>
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(
                      generateLayoutCodeExample(component, codeLanguage)
                    );
                    setCopied(true);
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="space-y-8">
          {layoutType
            ? (() => {
                const component = layoutComponentsData.find(
                  (comp) => comp.id === layoutType
                );
                if (!component) return <div>Layout component not found</div>;
                return <LayoutCard component={component} />;
              })()
            : layoutComponentsData.map((component) => (
                <LayoutCard key={component.id} component={component} />
              ))}
        </div>
      </div>
    </div>
